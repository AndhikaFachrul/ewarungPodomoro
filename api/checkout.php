...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //Ambil data keranjang dengan JOIN ke tabel varian_barang
    $query_cart = "SELECT c.id_cart, c.id_varian, c.qty, v.nama_satuan, v.harga, v.isi_per_satuan, b.id_barang, b.nama_barang, b.stok 
                   FROM cart c 
    ...
    //Cek Ketersediaan Stok (Berdasarkan jumlah x isi_per_satuan)
    foreach ($cart_items as $item) {
        $pengurangan_stok = $item['qty'] * $item['isi_per_satuan'];
        if ($item['stok'] < $pengurangan_stok) {
            echo json_encode(["status" => false, "message" => "Stok " . $item['nama_barang'] . " (" . $item['nama_satuan'] . ") tidak mencukupi!"]);
            exit();
        }
    }
...
try {
        ...
        $stmt_stok = mysqli_prepare($conn, "UPDATE barang SET stok = stok - ? WHERE id_barang = ?");
        ...
        foreach ($cart_items as $item) {
            ...
            // Potong stok
            mysqli_stmt_bind_param($stmt_stok, "ii", $pengurangan_stok, $item['id_barang']);
